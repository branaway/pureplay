...
def build(app, args, env):
    ...
    for (dirpath, dirnames, filenames) in os.walk(app.path):
        if dirpath == dist_dir:
            continue
        if dirpath.find(os.sep + '.') > -1 or dirpath.find(os.sep + 'tmp' + os.sep) > -1  or dirpath.find(os.sep + 'test-result' + os.sep) > -1 or dirpath.find(os.sep + 'logs' + os.sep) > -1 or dirpath.find(os.sep + 'eclipse' + os.sep ) > -1 or dirpath.endswith(os.sep + 'test-result') or dirpath.endswith(os.sep + 'logs')  or dirpath.endswith(os.sep + 'eclipse') or dirpath.endswith(os.sep + 'nbproject'):
            continue
