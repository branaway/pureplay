...
/**
 * The application classLoader. Load the classes from the application Java
 * sources files.
 */
public class ApplicationClassloader extends ClassLoader {
	...
	public Class<?> loadApplicationClass(String name) {
		// System.out.println("loading application class: " + name);
	...
	/**
	 * bran: exclude the Dispatcher in this run Detect Java changes
	 */
	public void detectChanges() {
		...
		Set<ApplicationClass> changedFromPlugins = new HashSet<ApplicationClass>();
		...
		for (ApplicationClass applicationClass : modifiedWithDependencies) {
			if (applicationClass.compile() == null) {
				...
				// bran hack
				// applicationClass.javaClass = new
				// ApplicationClassloader().defineClass(applicationClass.name,
				// applicationClass.enhancedByteCode, 0,
				// applicationClass.enhancedByteCode.length, protectionDomain);
				// end of hack
		...
		// bran: second pass
		// give the plugina seconds chance to work on the compile class
		if (modifiedWithDependencies.size() > 0)
			for (PlayPlugin plugin : Play.plugins) {
				changedFromPlugins.addAll(plugin.onNewlyCompiled(modifiedWithDependencies));
			}
		// //

		// now compile the new contributions
		for (ApplicationClass applicationClass : changedFromPlugins) {
			if (applicationClass.compile() == null) {
				Play.classes.classes.remove(applicationClass.name);
			} else {
				BytecodeCache.cacheBytecode(applicationClass.enhancedByteCode, applicationClass.name, applicationClass.javaSource);
	...
	private Pattern scanPattern = Pattern.compile("\\s+class\\s([a-zA-Z0-9_]+)\\s+");

	/**
	 * bran: seems to be it scans for the class name in each java file.
	 * 
	 * @param buf
	 * @param current
	 */
	void scan(StringBuffer buf, VirtualFile current) {
		if (!current.isDirectory()) {
			if (current.getName().endsWith(".java")) {
				// kind of expensive to read all the content
				Matcher matcher = scanPattern.matcher(current.contentAsString());
	...
	public void reloadStaticRouting() {
		// Now check for file modification
		// TODO add check to see if ther is any change in controllers and
		// router.
		if (StaticRouterPlugin.stateChanged() || !StaticRouterPlugin.routerFile.exists()) {
			StaticRouterPlugin.rebuild();
			StaticRouterPlugin.resetState();

			ApplicationClass applicationClass = Play.classes.getApplicationClass(StaticRouterPlugin.DISPATCHER_CLASS);
			applicationClass.refresh(); // reload the source
//
//			if (applicationClass.compile() == null) {
//				Play.classes.classes.remove(applicationClass.name);
//			} else {
//				BytecodeCache.cacheBytecode(applicationClass.enhancedByteCode, applicationClass.name, applicationClass.javaSource);
//				// applicationClass.javaClass =
//				// loadApplicationClass(StaticRouterPlugin.DISPATCHER_CLASS);
//				// ClassDefinition classDefinition = new
//				// ClassDefinition(applicationClass.javaClass,
//				// applicationClass.enhancedByteCode);
//				//
//				// Cache.clear();
//				// if (HotswapAgent.enabled) {
//				// try {
//				// HotswapAgent.reload(classDefinition);
//				// } catch (ClassNotFoundException e) {
//				// throw new UnexpectedException(e);
//				// } catch (UnmodifiableClassException e) {
//				// throw new UnexpectedException(e);
