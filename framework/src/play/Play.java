...
public class Play {
	...
	static void readConfiguration() {
		...
		// Include
		Map toInclude = new HashMap<String, String>();
		for (Object key : configuration.keySet()) {
	...
	public static synchronized void start() {
		...
		try {
			...
			if (mode == Mode.DEV) {
				...
				classloader = new ApplicationClassloader();
				// XXX shall we create a new ApplicationClasses instance?
...
//			StaticRouterPlugin.rebuild();
	...
	static boolean reDetection = true;
	 ...
	 * @return
	 */
	public static boolean doDetect() {
		try {
			for (PlayPlugin plugin : plugins) {
...
//			} catch (Exception e1) {
//				if (e1 instanceof PlayException)
//					throw e1;
//				else {
//					e = e1;
//				}
//			}
			...
			if (conf.lastModified() > startedAt) {
				start();
				return reDetection;
//				e = new RuntimeException("conf changed");
			}
...
//				throw e;
		...
		// bran added second pass to pick up static routing
		finally {
			classloader.reloadStaticRouting();
		}
		return skipDetection;
	}
